...
public abstract class NumberRunParseRule implements Rule<StringReader, String> {
    ...
    public @Nullable String parse(ParseState<StringReader> p_410712_) {
        StringReader stringreader = p_410712_.input();
        ...
        int k = j - i;
        if (k == 0) {
            p_410712_.errorCollector().store(p_410712_.mark(), this.noValueError);
            return null;
        } else if (s.charAt(i) != '_' && s.charAt(j - 1) != '_') {
            stringreader.setCursor(j);
            return s.substring(i, j);
        } else {
            p_410712_.errorCollector().store(p_410712_.mark(), this.underscoreNotAllowedError);
            return null;
        }
    }
