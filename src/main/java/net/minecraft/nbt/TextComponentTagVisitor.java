...
public class TextComponentTagVisitor implements TagVisitor {
    ...
    private static final EnumChatFormatting SYNTAX_HIGHLIGHTING_KEY = EnumChatFormatting.AQUA;
    private static final EnumChatFormatting SYNTAX_HIGHLIGHTING_STRING = EnumChatFormatting.GREEN;
    private static final EnumChatFormatting SYNTAX_HIGHLIGHTING_NUMBER = EnumChatFormatting.GOLD;
    private static final EnumChatFormatting SYNTAX_HIGHLIGHTING_NUMBER_TYPE = EnumChatFormatting.RED;
    ...
    private static final IChatComponent FOLDED = Component.literal("<...>").withStyle(EnumChatFormatting.GRAY);
    private static final IChatComponent BYTE_TYPE = Component.literal("b").withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private static final IChatComponent SHORT_TYPE = Component.literal("s").withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private static final IChatComponent INT_TYPE = Component.literal("I").withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private static final IChatComponent LONG_TYPE = Component.literal("L").withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private static final IChatComponent FLOAT_TYPE = Component.literal("f").withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private static final IChatComponent DOUBLE_TYPE = Component.literal("d").withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private static final IChatComponent BYTE_ARRAY_TYPE = Component.literal("B")
        .withStyle(SYNTAX_HIGHLIGHTING_NUMBER_TYPE);
    private final String indentation;
    private int indentDepth;
    private int depth;
    private final Component result = Component.empty();
    ...
    public void visitByte(ByteTag p_178258_) {
        this.result.append(Component.literal(String.valueOf(p_178258_.value())).withStyle(SYNTAX_HIGHLIGHTING_NUMBER))
            .append(BYTE_TYPE);
    }

    @Override
    public void visitShort(ShortTag p_178278_) {
        this.result.append(Component.literal(String.valueOf(p_178278_.value())).withStyle(SYNTAX_HIGHLIGHTING_NUMBER))
            .append(SHORT_TYPE);
    }
    ...
    public void visitLong(LongTag p_178276_) {
        this.result.append(Component.literal(String.valueOf(p_178276_.value())).withStyle(SYNTAX_HIGHLIGHTING_NUMBER))
            .append(LONG_TYPE);
    }

    @Override
    public void visitFloat(FloatTag p_178266_) {
        this.result.append(Component.literal(String.valueOf(p_178266_.value())).withStyle(SYNTAX_HIGHLIGHTING_NUMBER))
            .append(FLOAT_TYPE);
    }

    @Override
    public void visitDouble(DoubleTag p_178262_) {
        this.result.append(Component.literal(String.valueOf(p_178262_.value())).withStyle(SYNTAX_HIGHLIGHTING_NUMBER))
            .append(DOUBLE_TYPE);
    }
    ...
    public void visitByteArray(ByteArrayTag p_178256_) {
        ...
        for (int i = 0; i < abyte.length && i < 128; i++) {
            Component mutablecomponent = Component.literal(String.valueOf(abyte[i]))
    ...
    public void visitIntArray(IntArrayTag p_178268_) {
        ...
        for (int i = 0; i < aint.length && i < 128; i++) {
            this.result.append(" ")
                .append(Component.literal(String.valueOf(aint[i])).withStyle(SYNTAX_HIGHLIGHTING_NUMBER));
            if (i != aint.length - 1) {
                this.result.append(ELEMENT_SEPARATOR);
            }
        }
    ...
    private static boolean shouldWrapListElements(ListTag p_tag) {
        if (p_tag.size() < 8) {
            for (Tag tag : p_tag) {
    ...
    public void visitList(ListTag p_178272_) {
        ...
        } else if (!shouldWrapListElements(p_178272_)) {
            this.result.append("[");
    ...
    public void visitCompound(CompoundTag p_178260_) {
        if (p_178260_.isEmpty()) {
            ...
            if (LOGGER.isDebugEnabled()) {
                List<String> list = Lists.newArrayList(p_178260_.keySet());
